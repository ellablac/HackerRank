...
# this is a valid alternative: {1,2,3,4,1,2,3,4}, and it can be found
# in a single pass through the scores.
# 
# This solution breaks down when a non-decreasing sequence is immediately followed by
...
# technique described above, but 4 (the last candy element of the increasing sequence)
# must be reaplaced by 6. To achieve this in a single pass, we can add 4, 
# and then later compensate for the shortage by adding 2 (6-4).
...
def candies(n, arr):
    ...
    for i in range (1, n):
        ...
        if direc != "DOWN" and prev_dir == "DOWN":
            TotalCandies += max (0, candies + 1 - saved_cand)
            saved_cand = 1
